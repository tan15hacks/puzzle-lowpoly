...
const resolveScreenDirectionalGravity = (
  ...
  CARDINAL_DIRECTIONS.forEach((candidate) => {
    // Every shift should produce a new gravity state rather than replaying the
    // direction the robot is already falling toward.
...
/**
 * Adapts the chamber-rotation engine so SHIFT arrows describe the direction
 * the robot will visibly fall on screen. The chamber still performs its full
 * 90-degree rotation, but gravity is snapped to the chamber axis whose final
 * world-space projection best matches the pressed arrow.
 */
